...
Die Umsetzung der Wissenserwerbskomponente erfolgt am Beispiel von \textit{PaaSfinder}\footnote{{\label{foot:paasfinder}}https://paasfinder.org}, einem Expertensystem und Open-Source-Projekt im Bereich von \acf{PaaS}. Das Ziel des Systems besteht darin, zahlreiche PaaS-Anbieter (im Weiteren Vendors) vergleichbar zu machen. \textit{PaaSfinder} verfügt bereits über eine umfangreiche Wissensdatenbank, die aufgrund der häufigen Änderungen im gegebenen Anwendungsbereich regelmäßig aktualisiert werden muss. Die Struktur von \textit{PaaSfinder} lässt sich wie in Abbildung \ref{fig:paasfinder} folgendermaßen beschreiben.
\begin{figure}[H] 
	\centering
...
\begin{enumerate}
\item \textbf{General Properties}: allgemeine Eigenschaften des Vendors (z.B. Name, URL, Status, Typ etc.)
\item \textbf{Extensible}: generelle Erweiterungsmöglichkeit nach Kundenbedarf (z.B. spezielle Laufzeitumgebung)
\item \textbf{Pricing}: verfügbare Preismodelle
\item \textbf{Quality of Service}: die Servicequalität (z.B. Verfügbarkeit)
\item \textbf{Hosting}: Art der Bereitstellung (z.B. privat)
\item \textbf{Scaling}: Skalierbarkeit (z.B. Speichererweiterung)
\item \textbf{Runtimes}: unterstützte Laufzeitumgebungen (z.B. Java\footnote{https://java.com})
...
\end{enumerate}
Ein Vendor wird als \acs{JSON}\footnote{\label{foot:json}http://json.org}-Eintrag gespeichert. JSON stellt ein Datenaustauschformat dar und basiert auf zwei Datenstrukturen: 
\begin{itemize}
\item \textit{Name/Wert Paar}, das ein Objekt darstellt (siehe Abbildung \ref{fig:json-object}).
\item \textit{Eine geordnete Liste von Werten (Array)}, die kein oder mehrere durch Komma getrennte Objekte enthält (siehe Abbildung \ref{fig:json-array}).
\end{itemize}
\begin{figure}[H] 
	\centering
